...
void compute_multilevel_partition(const Tail &tail, const Head &head,
  ...
  auto check_if_better = [&] {
    ...
    if (current_tree_width < smallest_known_treewidth) {
      ...
      on_new_multilevel_partition(cells,
                                  open_cells.empty() ||
                                      max_closed_bag_size >= max_open_bag_size);
    }
  };
...
FlowCutter::FlowCutter(d4::SpecManagerCnf *specs)
    : head(specs->getNbVariable() + 1,
           specs->getSumSizeClauses() * specs->getSumSizeClauses() + 1),
      tail(head) {}
void FlowCutter::create_primal(std::vector<d4::Var> vars) {
  std::vector<unsigned int> clauses;
  om->getCurrentClauses(clauses, vars);
  int v_cnt = clauses.size();
  head.preimage_count_ = v_cnt;
  tail.preimage_count_ = v_cnt;
  int next_arc = 0;
  for (auto ci : clauses) {
    auto c = om->getClause(ci);
    for (int i = 0; i < c.size(); i++) {
      if (om->litIsAssigned(c[i]))
        continue;
      for (int j = i + 1; j < c.size(); j++) {
        if (om->litIsAssigned(c[j]))
          continue;
        head[next_arc] = c[i].var() - 1;
        tail[next_arc] = c[j].var() - 1;
        next_arc++;
        head[next_arc] = c[j].var() - 1;
        tail[next_arc] = c[i].var() - 1;
      }
    }
  }
  head.image_count_ = next_arc;
  tail.image_count_ = next_arc;
}
void FlowCutter::update_best_order(ArrayIDIDFunc &tail, ArrayIDIDFunc &head,
  ...
  auto is_left_subset_of_right = [](const std::vector<int> &l,
  ...
  auto compute_intersection_size = [](const std::vector<int> &l,
                                      const std::vector<int> &r) {
    auto i = l.begin(), j = r.begin();
    int n = 0;
    for (;;) {
  ...
  auto on_new_potential_maximal_clique = [&](int lowest_node_in_clique,
  ...
  best_tree.bags.resize(bag_count);
  best_tree.adj.resize(bag_count);
  best_tree.width = maximum_bag_size;

  for (int i = 0; i < bag_count; ++i) {
    best_tree.bags[i].resize(0);
    best_tree.adj[i].resize(0);
    for (auto x : nodes_in_bag[i])
      best_tree.bags[i].push_back(order(x) + 1);
  }

  {
    auto output_backbone_edge = [&](int b, int p) {
      best_tree.adj[b].push_back(p);
      best_tree.adj[p].push_back(b);
    };
    ...
    for (int b = 0; b < bag_count; ++b) {
      ...
      for (auto p : neighbor_bags) {
        if (p != b) {
          ...
          head.push_back(p);
          weight.push_back(
              compute_intersection_size(nodes_in_bag[b], nodes_in_bag[p]));
        }
      }
    }
...
void FlowCutter::decompose(std::vector<d4::Var> &vars) {
  long long start_time = get_milli_time();
  {
  ...
  auto on_new_multilevel_partition =
      [&](const std::vector<Cell> &multilevel_partition, bool must_print) {
        ...
        {
          best_bag_size = tw;
          update_best_partition(tail, head, preorder, multilevel_partition);
        }
  ...
  try {
    ...
    if (node_count > 500000) {
      ...
      compute_multilevel_partition(tail, head,
...

    {
      ...
      for (int i = 2;; ++i) {
        ...
        if (get_milli_time() - start_time > timeout) {
          return;
        }
      }
    }
...
int FlowCutter::compute_max_bag_size_of_order(const ArrayIDIDFunc &order) {
  auto inv_order = inverse_permutation(order);
  int current_tail = -1;
...
void FlowCutter::test_new_order(const ArrayIDIDFunc &order) {
  int x = compute_max_bag_size_of_order(order);
  {
    if (x < best_bag_size) {
      best_bag_size = x;
      update_best_order(tail, head, order);
      {
...
void FlowCutter::update_best_partition(ArrayIDIDFunc &tail, ArrayIDIDFunc &head,
  ...
  best_tree.bags.resize(bag_count);
  best_tree.adj.resize(bag_count);
  best_tree.width = get_treewidth_of_multilevel_partition(cell_list);

  for (int i = 0; i < bag_count; ++i) {
    best_tree.bags[i].resize(0);
    best_tree.adj[i].resize(0);
    for (auto &x : cell_list[i].separator_node_list)
      best_tree.bags[i].push_back(to_input_node_id(x));
  ...
  for (int i = 0; i < bag_count; ++i) {
    if (cell_list[i].parent_cell != -1) {
      int a = (i);
      int b = cell_list[i].parent_cell;
      best_tree.adj[a].push_back(b);
      best_tree.adj[b].push_back(a);
    }
  }
}
