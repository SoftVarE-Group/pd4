...
#include "HyperGraphExtractorPrimalProj.hpp"
 ...
 */
HyperGraphExtractorPrimalProj::HyperGraphExtractorPrimalProj(
    unsigned nbVar, unsigned nbClause) {
...
*/
void HyperGraphExtractorPrimalProj::clashHyperEdgeIndex(
 ...
 */
void HyperGraphExtractorPrimalProj::extractCutFromEdges(
...
/**
   ...
   @param[out] cutSet, the computed cutset.
*/
void HyperGraphExtractorPrimalProj::extractCutFromHyperGraph(
 ...
 */
void HyperGraphExtractorPrimalProj::removeSubsumEdges(HyperGraph &hypergraph) {
  unsigned *edge = hypergraph.getEdges();
  for (unsigned i = 0; i < hypergraph.getSize(); i++) {
    if (m_hashEdges[i]) {
  ...
  for (i = j = 0; i < hypergraph.getSize(); i++) {
    ...
    if (m_hashEdges[i]) {
      ...
      hypergraph.getCost()[j] = hypergraph.getCost()[i];
...
/**
   ...
   @param[out] hypergraph, the computed hyper graph.
 */
void HyperGraphExtractorPrimalProj::constructHyperGraph(
  ...
  std::vector<bool> clean_vars(om.nbSelected() + 1, true);

  clean_vars.resize(om.getNbVariable() + 1, false);
  for (auto &v : equivVar) {
    bool clean = false;
    for (auto vv : v) {
      if (om.isSelected(vv)) {
    ...
    *edge = 0;
    ...
    for (auto &l : om.getClause(idx)) {
      if (!om.litIsAssigned(l) && !m_markedVar[equivClass[l.var()]]) {
        ...
        clean += clean_vars[l.var()];
        cnt++;
      }
    }
    int cost = 1;
    std::cout<<"Clean "<<idx<<std::endl;
  ...
  std::cout<<"Clean cl: "<<all_clean<<std::endl;

  // remove useless edges.
  if (reduceFormula && false)
