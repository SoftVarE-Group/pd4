...
// Graph
Graph::Graph(int vars, const vector<vector<Glucose1::Lit>>& clauses)
{
	clear();
	init(vars);
...
Graph::Graph(int vars, const vector<vector<Glucose1::Lit>>& clauses, const vector<vector<Glucose1::Lit>>& learnts, vector<int>& freq)
{
	clear();
	init(vars);
